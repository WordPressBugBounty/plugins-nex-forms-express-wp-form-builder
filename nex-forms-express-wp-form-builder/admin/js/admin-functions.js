...
function()
	...
	jQuery(document).on('click','.new-form-sidebar li:not(.disabled) a, .new-form-sidebar2 li:not(.disabled) a',
			function()
				{
	...
	jQuery(document).on('click','.new-report-container .new-form-sidebar2 li:not(.disabled) a',
			function()
				{
				jQuery('.new-report-container .new-form-sidebar2 li').removeClass('active');
				jQuery(this).parent().addClass('active');
				
				jQuery('.new-report-panel').removeClass('active');
				jQuery('.new-report-panel.'+jQuery(this).attr('data-panel')).addClass('active');
	...
	jQuery(document).on('click','#upload_form',
			function()
				{
	...
	if(typeof jQuery({}).ajaxForm == 'function')
		{
			...
			jQuery('#new_nex_form').ajaxForm({
					...
					beforeSubmit: function(formData, jqForm, options) {
						jQuery(this).parent().find('.nf-alert').remove();	
		
						if(jQuery('input#form_title').val()=='')
							{
							jQuery('<div class="alert alert-danger nf-alert">Form title can not be empty. Please enter a form title.</div>').insertBefore('input#form_title');
							
							jQuery('input#form_title').focus();
							return false;
							}
						else
							{
	...
	jQuery(document).on('click','.create_new_form, .create_new_form_home',
		function()
			{
			if(jQuery(this).hasClass('create-template'))
				{
				jQuery('#new_form_setup a[data-panel="panel-2"]').trigger('click');
				jQuery('#new_form_setup a[data-panel="panel-2"]').parent().addClass('active');
				}
				
			else if(jQuery(this).hasClass('do-tut'))
				{
				jQuery('#new_form_setup a[data-panel="panel-3"]').trigger('click');
				jQuery('#new_form_setup a[data-panel="panel-3"]').parent().addClass('active');
				}
			else
				{
				
				jQuery('#new_form_setup a[data-panel="panel-1"]').trigger('click');	
				jQuery('#new_form_setup a[data-panel="panel-1"]').parent().addClass('active');
				}
...
function()
	...
	setTimeout(function(){set_c_logic_fields();
	...
	/*jQuery(document).on('click','.rule_number',function() {
	var rule_key = jQuery(this).closest('.new_rule').attr('data-rule-key');
	  
	  var get_offset = jQuery('.conditional-logic-opened .for_rule_'+rule_key).offset();
	  
	  if(get_offset)
	  	{
		  console.log(get_offset.top);
...
function set_c_logic_fields(the_select){
	...
	set_current_action_fields_conditional_logic += '<optgroup label="Grids" class="cl_grids">';
	//SPACER
					//set_current_action_fields_conditional_logic += '<option class="option_spacer" disabled="disabled">&nbsp;</option>';
	jQuery('div.nex-forms-container div.form_field.grid-system').each(
		function()
			{
			if(jQuery(this).attr('data-grid-name'))
				{
				set_current_action_fields_conditional_logic += '<option  data-field-id="'+ jQuery(this).attr('id') +'" data-field-name="grid" data-field-type="panel"   value="'+ jQuery(this).attr('id') +'**grid##grid">'+ short_str(jQuery(this).attr('data-grid-name')) +'</option>';	
				}
	...
	jQuery('div.nex-forms-container div.form_field.is_panel').each(
		function()
			{
			set_current_action_fields_conditional_logic += '<option  data-field-id="'+ jQuery(this).closest('.form_field').attr('id') +'" data-field-name="panel" data-field-type="panel"   value="'+ jQuery(this).attr('id') +'**panel##panel">'+ ((jQuery(this).find('.panel').attr('data-panel-id') && jQuery(this).find('.panel').attr('data-panel-id')!='') ? jQuery(this).find('.panel').attr('data-panel-id') : short_str(jQuery(this).find('.panel-heading').text())) +'</option>';
			//SPACER
					//set_current_action_fields_conditional_logic += '<option class="option_spacer" disabled="disabled">&nbsp;</option>';
			}
		);	
	...
	//SPACER
					//set_current_action_fields_conditional_logic += '<option class="option_spacer" disabled="disabled">&nbsp;</option>';
	jQuery('div.nex-forms-msg-container div.form_field.html_fields').each(
		function()
			{
