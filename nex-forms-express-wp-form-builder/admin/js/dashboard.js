		...
		if ((event.ctrlKey || event.metaKey) && !event.shiftKey)
				...
				switch (String.fromCharCode(event.which).toLowerCase())
					...
					case 'a':
						...
						jQuery('.wap_nex_forms_entries tbody tr').addClass('active');
						
						var selection_array = [];
						jQuery('.wap_nex_forms_entries tbody tr.active').each(
							function()
								{
...
function NEXForms_reset_entries_menu(){
	...
	jQuery('.form_entry_data').html('');
	
	jQuery('.entry_views button').prop('disabled','disabled');
	jQuery('.entry_actions button').prop('disabled','disabled');
...
function NEXForms_restore_entry(){
	...
	var data =
		...
		nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
		selection						: selection_array,
		};
	jQuery.post
		(
...
(function($)
	...
	$(document).ready(function()
		...
		jQuery('.database_table.wap_nex_forms_add_ons.dashboard-box tbody tr').each(
			function()
				{
				var get_status = jQuery(this).find('span.add-on-status');
				if(get_status.hasClass('activated'))
					jQuery(this).addClass('activated');	
				}
			);
		
		jQuery(document).on('click','.install_add_on', //
			function()
				{
				var btn = jQuery(this);
				var add_on_path = jQuery(this).attr('data-add-on-path');
				var do_action = jQuery(this).attr('data-do-action');
				var add_on_id = jQuery(this).attr('data-add-on-id');
				var btn_text = jQuery(this).html();
				
				jQuery('.add_on_error').remove();
				
				if(do_action=='install')
					btn_text = '<span class="fa fa-solid fa-spinner fa-spin"></span> Installing';	
				if(do_action=='activate')
					btn_text = '<span class="fa fa-solid fa-spinner fa-spin"></span>';	
				...
				jQuery.ajax({
						...
						type: 'POST',
						data: {
							action: 'nexforms_install_addon', 
							do_action: do_action,
							add_on_id: add_on_id,
							plugin_file: add_on_path,
							nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
							addon_url: 'https://basixonline.net/nex-forms-add-ons-package/'+ add_on_path +'.zip' 
						},
						success: function(response) {
							...
							if(do_action=='install')
								{
								if(response.data.message=='no_plan' || response.data=='no_plan')
									{
									jQuery('.col-sm-9').prepend('<div class="alert alert-danger add_on_error"><span class="fas fa-lock"></span> PREMIUM ADD-ON ONLY -  To unlock this Add-On you need to <a href="https://basixonline.net/nex-forms/pricing/" target="_blank"><strong>upgrade to PREMIUM</strong></a></div>');
									btn_text = 'Install';
									//alert(response.data.message || response.data);	
									}
								else
									{
									btn_text = 'Deactivate';	
									btn.attr('data-do-action','deactivate');
									btn.closest('tr').addClass('activated');
									btn.closest('tr').find('.add-on-status').text('Active');
									btn.closest('tr').find('.add-on-status').attr('class','add-on-status activated');
									}
								}
							if(do_action=='activate')
								{
							...
							if(do_action=='deactivate')
								{
								btn_text = 'Activate';
								btn.attr('data-do-action','activate');
								btn.closest('tr').removeClass('activated');
								btn.closest('tr').find('.add-on-status').text('Inactive');
								btn.closest('tr').find('.add-on-status').attr('class','add-on-status inactive');
								}
							...
							jQuery.ajax({
									...
									type: 'POST',
									data: {
										action: 'nexforms_get_add_on_stats', 
										do_ajax: true, 
									},
									success: function(response) {
										
										jQuery('.stats_summary_container3 .header_stats2').html(response);
						...
						error: function(err) {
							btn.html('Install');
							jQuery('.col-sm-9').prepend('<div class="alert alert-danger add_on_error">Installation Failed -  Please <a href="https://basixonline.net/add-ons/nex-forms-add-on-package.zip" target="_blank">click here to download all add-ons<strong> and then install each add-on from your WP Plugins page.</strong></a></div>');
							console.log(err);
						}
					});
				}
		...
		var ell = jQuery('#fs_connect').detach();
		jQuery('.activation_box').append(ell);
		ell.css('display','block');
		
		jQuery(document).on('input','input#report_title', //
			function()
				{
				jQuery('.report-name').text(' - ' + jQuery(this).val());	
				}
			);
		jQuery(document).on('click','td .open_report', //
			function()
				{
				jQuery('.report_steps').html('<div class="ajax_loading active"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>');
				var data =
							{
							action	 						: 'nf_edit_report',
							do_ajax							: true,
							report_update_id				: jQuery(this).attr('id'),
							nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
							};
						jQuery.post
							(
							ajaxurl, data, function(response)
								{
								jQuery('.report_steps').html(response);
								update_select('.form_selection');
								jQuery('.report-name').text(' - ' + jQuery('input#report_title').val());
								
								jQuery('.report-bc').text('Report');
								
								if(jQuery('#report-status').text()=='3')
									{
									jQuery('a[data-panel="panel-1"]').closest('li').removeClass('disabled');
									jQuery('a[data-panel="panel-2"]').closest('li').removeClass('disabled');
									jQuery('a[data-panel="panel-3"]').closest('li').removeClass('disabled');
									jQuery('a[data-panel="panel-3"]').trigger('click');
									jQuery('.panel-3 .run_query').trigger('click');
									
									if(jQuery('.clause_container .new_clause').is('div'))
										{
										jQuery('.open_reporting_filters.nf_button').addClass('active');
										jQuery('.new-report-container').addClass('open-report-filters');
										}
									else
										{
										jQuery('.open_reporting_filters.nf_button').removeClass('active');	
										jQuery('.new-report-container').removeClass('open-report-filters');	
										}
									
									}
								if(jQuery('#report-status').text()=='2')
									{
									jQuery('a[data-panel="panel-1"]').closest('li').removeClass('disabled');
									jQuery('a[data-panel="panel-2"]').closest('li').removeClass('disabled');
									jQuery('a[data-panel="panel-3"]').closest('li').addClass('disabled');
									jQuery('a[data-panel="panel-2"]').trigger('click');
									}
		...
		jQuery(document).on('click','.open_reporting_filters', //
			function()
				{
		...
		jQuery(document).on('click','.close_filters', //
			function()
				{
				jQuery('.new-report-container').removeClass('open-report-filters');
				jQuery('.open_reporting_filters.nf_button').removeClass('active');
		...
		jQuery(document).on('click','.new_report', //
			function()
				{
				jQuery('.new-report-panel').find('.nf-alert').remove();	
				jQuery('#report-edit-Id').remove();
				jQuery('.form_selection').attr('data-selected','0');
				update_select('.form_selection');
				jQuery('.new-report-panel').find('input#report_title').val('');
				jQuery('.new-form-sidebar2 li').addClass('disabled');
				jQuery('a[data-panel="panel-1"]').closest('li').removeClass('disabled');
				jQuery('a[data-panel="panel-1"]').trigger('click');
				jQuery('.create_new_report').val('Next');
				jQuery('.report-name').text('');
				jQuery('.report-bc').text('New Report');
				}
			);
		
		jQuery(document).on('click','.create_new_report', //
			function()
				{
				jQuery('.new-report-panel').find('.nf-alert').remove();	
				if(jQuery('input#report_title').val()=='' || jQuery('select.form_selection option:selected').val()=='0')
					{
					if(jQuery('input#report_title').val()=='')
						{
						jQuery('<div class="alert alert-danger nf-alert">Report Title can not be empty. Please enter a report title.</div>').insertBefore('input#report_title');
						jQuery('input#report_title').focus();
						}
					if(jQuery('select.form_selection option:selected').val()=='0')
						{
						jQuery('<div class="alert alert-danger nf-alert">Please select a Reporting Form.</div>').insertBefore('select.form_selection');
						}
					}
				else
					{
					jQuery('.new-report.panel-2').html('<div class="ajax_loading active"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>');
					var update = ((jQuery('#report-edit-Id').text()!='') ? jQuery('#report-edit-Id').text() : 0);
						var data =
							{
							action	 						: 'nf_create_new_report',
							report_title					: jQuery('input#report_title').val(),
							report_form						: jQuery('select.form_selection option:selected').val(),
							report_update_id				: update,
							nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
							};
						jQuery.post
							(
							ajaxurl, data, function(response)
								...
								if(strstr(response,'exists'))
									jQuery('<div class="alert alert-danger nf-alert">'+ response + '</div>').insertBefore('input#report_title');
								else
									{
									jQuery('a[data-panel="panel-2"]').closest('li').removeClass('disabled');
									...
									jQuery('.new-report.panel-2').html(response);
									jQuery('.create_new_report').val('Save');
									
									nf_get_records(0,jQuery('.wap_nex_forms_reports').find('.paging_wrapper'));
		...
		jQuery(document).on('change','.toggel_field_selection', //
			function()
				{
				if(jQuery(this).prop('checked'))
					jQuery('input[name="showhide_fields[]"]').prop('checked',true);
				else
					jQuery('input[name="showhide_fields[]"]').prop('checked',false);
				}
			);
		
		
		jQuery(document).on('click','.report_field_row .col-2', //
			function()
				{
		...
		jQuery(document).on('click','.reporting-forms tr.form_record', //
			function()
				{
				jQuery('.reporting-forms tr.form_record').removeClass('active');
				jQuery(this).addClass('active');
				jQuery('.new-report.panel-3').html('<div class="ajax_loading active"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>');
				jQuery('a[data-panel="panel-3"]').closest('li').removeClass('disabled');
				jQuery('a[data-panel="panel-3"]').trigger('click');
				var data =
					{
					action	 						: 'nf_get_reporting_fields',
					form_id	 						: jQuery(this).attr('id'),
					report_update_id				: jQuery('#report-edit-Id').text(),
					nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
					};
				jQuery.post
					(
					ajaxurl, data, function(response)
						{
						jQuery('.new-report.panel-3').html(response);
						}
					);
				}
			);
		jQuery(document).on('click','.set_field_selection', //
			function()
				{
				...
				jQuery('.new-report.panel-3').html('<div class="ajax_loading active"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>');
				nf_get_records(0,jQuery('.wap_nex_forms_reports').find('.paging_wrapper'));
				nf_build_report_table2(jQuery('select.form_selection option:selected').val(),additional_params);
		...
		jQuery(document).on('mouseenter','.dash-left-col', //
			function()
				{
				jQuery(this).addClass('opened');
				}
			);
		jQuery(document).on('mouseleave','.dash-left-col', //
			function()
				{
		...
		update_select('.pref_date_picker_lang')
...
//BATCH SELECTION				
		...
		$(document).on('click','li.form_item',
			function(e)
				...
				if($(this).hasClass('all_entries'))
					...
					{
					additional_params += '{"column":"paypal_payment_id","operator":"IS NOT","value":"NULL"}, {"column":"trashed","operator":"IS","value":"NULL"}';
					}
				
				else if($(this).hasClass('paypal_entries_paid'))
					{
					additional_params += '{"column":"paypal_payment_id","operator":"IS NOT","value":"NULL"}, {"column":"payment_status","operator":"=","value":"payed"}, {"column":"trashed","operator":"IS","value":"NULL"}';
					}
				
				else if($(this).hasClass('paypal_entries_unpaid'))
					{
					additional_params += '{"column":"paypal_payment_id","operator":"IS NOT","value":"NULL"}, {"column":"payment_status","operator":"=","value":"failed"}, {"column":"trashed","operator":"IS","value":"NULL"}';
					}
				else if($(this).hasClass('paypal_entries_pending'))
					{
					additional_params += '{"column":"paypal_payment_id","operator":"IS NOT","value":"NULL"}, {"column":"payment_status","operator":"=","value":"pending"}, {"column":"trashed","operator":"IS","value":"NULL"}';
					}
		...
		$(document).on('click','.filter_add_on',
			function(e)
				{
				...
				if($(this).hasClass('active'))
					{
					$('.filter_add_on').removeClass('active');
					$(this).removeClass('active');
					}
				else
					{
				...
				if($(this).hasClass('active'))
					{
					if($(this).attr('id')=='filter_add_on_active')
						{
						additional_params += '{"column":"status","operator":"=","value":"active"}';
						}
					if($(this).attr('id')=='filter_add_on_inactive')
						{
						additional_params += '{"column":"status","operator":"=","value":"inactive"}';
						}
					if($(this).attr('id')=='filter_add_on_not_installed')
						{
						additional_params += '{"column":"status","operator":"=","value":"not installed"}';
						}
					}
				additional_params += ']';
				
				$('.wap_nex_forms_add_ons').find('input[name="additional_params"]').val(additional_params);
				
				nf_get_records(0,$('.wap_nex_forms_add_ons').find('.paging_wrapper'),'');
		...
		*/
		...
		if(jQuery('.form_analytics').hasClass('summary_stats'))
			{
			nf_print_chart('line', '0', 'summary');
			}
		else
			{
			nf_print_chart('line', '0', '');	
			}
...
/* SORT INTO APROPRIATE FILES */
	...
	$(document).on('click','th a span.sortable-column',
		...
		$(document).on('click','.run_query',
			function()
				...
				nf_build_report_table2(jQuery(this).attr('id'), additional_params, true);
		...
		$(document).on('click','.print_to_pdf',
			function()
				...
				var getnex_forms_wpnonce = jQuery('#nex_forms_wpnonce').text();
					...
					ajaxurl, data, function(response)
						{
							...
							setTimeout(function()
								...
								var data2 =
									...
									form_entry_Id					: record_id,
									nex_forms_wpnonce						: getnex_forms_wpnonce
									};	
								jQuery.post
									(
		...
		$(document).on('click','.print_report_to_pdf',
			function()
				...
				var data =
					...
					report_Id	 : jQuery('#report-edit-Id').text(),
					nex_forms_wpnonce: jQuery('input[name="nex_forms_wpnonce"]').val()
					};	
				Materialize.toast('Creating PDF, please wait', 60000, 'loading-pdf');
				jQuery.post
					(
		...
		$(document).on('click','.quickprint_report_to_pdf',
			function()
				{
				var getnex_forms_wpnonce = jQuery('#nex_forms_wpnonce').text();
				var data =
			...
			jQuery.post
				...
				ajaxurl, data, function(response)
					...
					if(strstr(response, 'License Activated'))
						{
						Materialize.toast('Purchase code Registered!', 5000, 'toast-success')
						setTimeout(function(){ jQuery(location).attr('href',jQuery('#siteurl').text()+'/wp-admin/admin.php?page=nex-forms-dashboard'), 3000});
						}
					else if(strstr(response, 'License already in use'))
						{
						jQuery('.show_code_response').html(response);
						if(jQuery('env_s_expired').text()!='true')
		...
		$(document).on('click','.delete-record',
			function()
				...
				if (result) 
					...
					if($(this).closest('.database_table').hasClass('wap_nex_forms_reports'))
						{
						var data =
							{
							action	 						: 'nf_delete_db_table',
							nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
							Id								: record_id
							};	
						jQuery.post(ajaxurl, data, function(response){});
	...
	$(document).on('change','select[name="stats_per_form"], select[name="stats_per_year"], select[name="stats_per_month"]',
		function()
			...
			jQuery('.analytics_form').text($('select[name="stats_per_form"] option:selected').text());
			jQuery('.analytic_month').text($('select[name="stats_per_month"] option:selected').text());
			jQuery('.analytic_year').text($('select[name="stats_per_year"] option:selected').text());
			}
		);
...
function nf_print_chart(chart_type, form_id, is_summary){
	...
	if(jQuery('.chart-container').attr('class'))	
	...
	jQuery.post
		...
		ajaxurl, data, function(response)
			...
			jQuery('.header_stats').remove();	
			var set_stats = jQuery('.stats').clone();
			jQuery('.stats_summary_container2').html('');
			jQuery('.stats_summary_container2').html(set_stats);
			
			set_stats.addClass('header_stats');
			//set_stats.insertAfter('.stats-controls');
			
			jQuery('.loaded_tc_geo_stats').remove();
			var set_top_countries = jQuery('.top_countries').clone();
			jQuery('.geo_stats_container').html('');
			jQuery('.geo_stats_container').html(set_top_countries);
			set_top_countries.addClass('loaded_tc_geo_stats');
			
			jQuery('.loaded_top_form_stats').remove();
			var set_top_forms = jQuery('.top_forms').clone();
			jQuery('.top_forms_container').html('');
			jQuery('.top_forms_container').html(set_top_forms);
			set_top_forms.addClass('loaded_top_form_stats');
		}
	);	
}
...
function nf_get_records(page,target,id,form_id){
	...
	jQuery.post
		...
		ajaxurl, data, function(response)
			...
			target.closest('.database_table').find('tbody.saved_records_contianer').html('<tr><td colspan="100"><div class="alert alert-danger"><span class="fas fa-lock"></span> PREMIUM ONLY FEATURE: An active premium license is required to view submitted form data. <a href="https://basixonline.net/nex-forms/pricing/" class="upgrade-link" target="_blank"> Upgrade to Premium <span class="fa-solid fa-angles-up"></span></a></td><tr>');
			...
			var total_records = target.closest('.database_table').find('.saved_records_container .total_table_records').text()
			...
			var output = '';
			//alert(target.closest('.database_table').find('.saved_records_container .total_table_records').text());
			output += '<span class="displaying-num"><span class="entry-count">'+ total_records +'</span> items </span>';
				...
				if(total_pages>1)
				{
				output += '<a title="Go to the first page" class="first-page iz-first-page btn waves-effect waves-light"><span class="fa fa-backward-step"></span></a>';
...
function nf_build_report_table2(form_id, additional_params, refresh_data){
	
	jQuery('a[data-panel="panel-1"]').closest('li').addClass('disabled');
	jQuery('a[data-panel="panel-2"]').closest('li').addClass('disabled');
	jQuery('.new-report.panel-3').html('<div class="ajax_loading active"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>');
	var data = {
		action							: 'submission_report2',
		form_Id							: form_id,
		additional_params				: additional_params,
		table							: 'nex_forms_'+ format_illegal_chars(jQuery('input#report_title').val()),
		report_update_id				: jQuery('#report-edit-Id').text(),
		nex_forms_wpnonce				: jQuery('#nex_forms_wpnonce').text(),
		refresh_data					: refresh_data,
		field_alias						: jQuery('.report_field_row').map(function() {
        return {
            field_name: jQuery(this).find('input[name^="field_selection"][name$="[field_name]"]').val(),
            col_name: jQuery(this).find('input[name^="field_selection"][name$="[col_name]"]').val()
        };
    }).get(),
		field_selection					: jQuery('input[name="showhide_fields[]"]:checked').map(function() {
			return jQuery(this).val();
		}).get(), // Collects all checked checkbox values into an array
	};  
	...
	jQuery.ajax({
		...
		type: 'POST',
		data: data,
		timeout: 600000, // 10 minutes (increase as needed)
		success: function(response) {
			jQuery('.new-report.panel-3').html(response);
			
				if(jQuery('.new-report-container').hasClass('open-report-filters'))
					{
					jQuery('.open_reporting_filters.nf_button').addClass('active');
					}
			...
			var data2 =
					{
					action	 						: 'nf_report_get_additional_params',
					form_Id							: form_id,
					additional_params				: additional_params,
					table							: 'nex_forms_'+ format_illegal_chars(jQuery('input#report_title').val()),
					report_update_id				: jQuery('#report-edit-Id').text(),
				...
				jQuery.post
					...
					ajaxurl, data2, function(response2)
						{		
						jQuery('.new-report.panel-3 .right-col-top').html(response2);
...
function()
	{
		...
		jQuery('#other-pref').ajaxForm({
		   ...
		   success : function(responseText, statusText, xhr, $form) {
			   ...
			   Materialize.toast('Form Preferences Saved', 2000, 'toast-success');
...
console.log(formattedDate); // Output: 2025-07-15
	
	var special_run = new Tour({
		  name: "se-" + formattedDate,
		  onStart: function(){ },
		  onEnd: function(){  },
		  steps: [
		  {
			orphan: true,
			title: "<span class=''>Envato Support Expired</span>",
			...
			content: '<h5>Your Envato support have expired, <strong>but...Great News!</strong></h5> We\'ve launched a powerful new version of NEX-Forms on a cloud based platform from Freemius, and it\'s packed with benefits you won\'t want to miss:<br><span class="fa fa-check txt-green"></span>&nbsp;Ongoing Support &amp; Updates<br><span class="fa fa-check txt-green"></span>&nbsp;Faster Performance &amp; Better Stability<br><span class="fa fa-check txt-green"></span>&nbsp;Easy License Management<br><span class="fa fa-check txt-green"></span>&nbsp;Access to New Premium Features<br><span class="fa fa-check txt-green"></span>&nbsp;Access to Up and Coming Premium add-ons<br><span class="fa fa-check txt-green"></span>&nbsp;No More Support Renewal Hassles<br><span class="fa fa-check txt-green"></span>&nbsp;Receive an extra FREE development license<br><h5>How do I upgrade?</h5>1. Just <a href="https://basixonline.net/nex-forms/pricing-comparison-envato-vs-SaaS/?promo=1">Click here and switch to a plan</a> that better suit your needs. <br>2. Go to the NEX-Forms Settings and enter your new license.<br><h5>Zero Data Loss</h5>No forms, form entries, settings or any data from your NEX-Forms setup will be lost when you transition to the cloud based platform. All your forms, form submissions, settings and data will remain exactly as it is now.<br><h5>Added Bonus!</h5>As an Envato License holder we will also offer you a <strong> 50% Discount</strong> on all pricing plans, simply click on the button below and claim your discount <span class="fa fa-arrow-down"></span>',
			//content: "<div class='new_tut' style=\"line-height: 32px;font-size:23px;color:#000 ;\"><strong style=\"color:#000 ;\">Our BIGGEST Cyber Monday / Black Friday Sale yet!!!</strong><br><br>Buy NEX-Forms today for <strong class=\"hl\"  style=\"color:#9315bd ;\">LESS 50%</strong><br>and ALSO get <strong style=\"color:#9315bd ;\" class=\"hl\">ALL ADD-ONS for FREE!</strong></h5></strong></span><span class='tip' style=\"font-size: 18px;line-height: 25px !important;margin-bottom: 0px;\">A<strong style=\"color:#9315bd ;\" class=\"hl\"> ONCE-OFF PURCHASE</strong> includes <strong style=\"color:#9315bd ;\" class=\"hl\">LIFETIME FREE UPDATES</strong> for all items and <strong style=\"color:#9315bd ;\" class=\"hl\">SUPPORT!</strong></span></div>",
			template: "<div class='popover tour tutorial-step-1 se-error'><div class='popover-arrow'></div><h3 class='popover-title se-notice'></h3><div class='popover-content se-notice'></div><div class='popover-footer se-notice'><a href=\"https://basixonline.net/nex-forms/pricing-comparison-envato-vs-SaaS/?promo=1\" target=\"_blank\" class=\"do_saas_upgrade\">Upgrade NOW with up to <strong>50% Discount</strong></a><a href=\"mailto:support@basixonline.net?subject=NEX-Forms Subscriptions Plan Question\" target=\"_blank\" class=\"do_saas_upgrade faq\">I have more questions</a></div><button class='end-tour' data-role='end'><span class='fa fa-close'></span></button></div>",
	...
	jQuery(document).on('click','.sup_ex',
			function()
				{
				special_run.init();
				special_run.restart();
