...
function()
	...
	jQuery(".set_form_width").bind('keyup input', function(){
	...
	*/
	...
	jQuery(".fw-bg-color").spectrum(
		...
		showInitial: "true",
		 ...
		 move: function(color)
			{
			current_field.css('background',jQuery(this).val())
			}
		});
	
	jQuery(".fw-brd-color").spectrum(
		{
		...
		showInitial: "true",
		 ...
		 move: function(color)
			{
			current_field.css('border-color',jQuery(this).val())
			}
		});
...
/* OVERALL FORM SETTINGS */
	...
	jQuery("#form_padding").bind('keyup input', function(){
...
//SETUP INPUT SETTINGS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
		...
		jQuery(document).on('keyup','div.field-settings-column #set_input_name',
			function()
				...
				if(current_field.hasClass('tags'))
					current_field.find('input').attr('name',formated_value)
	...
	//SET INPUT SIZE
		jQuery(document).on('click','.radio-check-size button',
			function()
				{
				jQuery('.radio-check-size button').removeClass('active');
				jQuery(this).addClass('active');
				
				var get_label = current_field.find('#the-radios');
				get_label.removeClass('radio-lg').removeClass('radio-sm');
				
				if(jQuery(this).hasClass('small'))
					get_label.addClass('radio-sm');
				if(jQuery(this).hasClass('large'))
					get_label.addClass('radio-lg');
				}
			);
	...
	jQuery(document).on('click','.bg-position button',
		...
		//DEVICE ONLY
		jQuery(document).on('click','.setting-device-only button',
			function()
				{
				//jQuery('.disabled_hours button').removeClass('active');
				jQuery(this).toggleClass('active');
				
				current_field.removeClass('nf-no-mobile');
				current_field.removeClass('nf-no-tablet');
				current_field.removeClass('nf-no-laptop');
				current_field.removeClass('nf-no-pc');
				
				var set_class = '';
				jQuery('.setting-device-only button').each(
					function()
						{
						if(!jQuery(this).hasClass('active'))
							{
							set_class += jQuery(this).attr('data-val')+' ';
							}
						}
					);
				
				current_field.addClass(set_class);
			...
			{	
					...
					for (var i = 0; i < items.length; i++)
						...
						if(items[i]!='')
							{
							//optgroup
							//if(strstr(items[i],'[group]') || strstr(items[i],'[end]'))
								//{
								if(strstr(items[i],'[group]'))
									{
									var get_item = items[i];
									var set_item = get_item.replace('[group]','');
									set_options += '<optgroup label="'+ set_item +'">';
									}
								else if(strstr(items[i],'[end]'))
									{
									set_options += '</optgroup>';
									}
...
jQuery(document).on('change', "#set_radios",
		function()
			...
			var get_label_style = current_field.find('.the-radios .input-label').attr('style');
			var get_radio_style = current_field.find('.the-radios a:not(.checked)').attr('style');
				...
				if(items[i]!='')
					...
					if(current_field.hasClass('multi-image-select-group') || current_field.hasClass('single-image-select-group'))
						...
						if(strstr(items[i],'=='))
							{
							var split_option = items[i].split('==')
							set_inputs += '<label class="checkbox-inline '+ set_layout +'" for="'+ format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i]) +'" ><span class="svg_ready"><input class="check the_input_element" type="checkbox" name="'+ format_illegal_chars(current_field.find('.the_label').text()) +'[]" id="'+format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i])+'" value="'+split_option[0]+'"><span class="input-label check-label" style="' +get_label_style+ '">'+split_option[1]+'</span></span></label>';
							}
						else
							set_inputs += '<label class="checkbox-inline '+ set_layout +'" for="'+ format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i]) +'" ><span class="svg_ready"><input class="check the_input_element" type="checkbox" name="'+ format_illegal_chars(current_field.find('.the_label').text()) +'[]" id="'+format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i])+'" value="'+items[i]+'"><span class="input-label check-label" style="' +get_label_style+ '">'+items[i]+'</span></span></label>';
						}
						...
						if(strstr(items[i],'=='))
							{
							var split_option = items[i].split('==')
							set_inputs += '<label class="radio-inline '+ set_layout +'" for="'+ format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i]) +'" ><span class="svg_ready"><input class="radio the_input_element" type="radio" name="'+ format_illegal_chars(current_field.find('.the_label').text()) +'" id="'+format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i])+'" value="'+split_option[0]+'"><span class="input-label radio-label" style="' +get_label_style+ '">'+split_option[1]+'</span></span></label>';
							}
						else
							set_inputs += '<label class="radio-inline '+ set_layout +'" for="'+ format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i]) +'" ><span class="svg_ready"><input class="radio the_input_element" type="radio" name="'+ format_illegal_chars(current_field.find('.the_label').text()) +'" id="'+format_illegal_chars(current_field.find('.the_label').text())+'_'+format_illegal_chars(items[i])+'" value="'+items[i]+'"><span class="input-label radio-label" style="' +get_label_style+ '">'+items[i]+'</span></span></label>';
						}
					}
				}
			...
			current_field.find( "#the-radios a" ).attr('style',get_radio_style);			
			}
	...
	//SET RADIO BG COLOR UNCHECKED
		change_color('set-radio-bgc-color-uc','data-checked-bg-color-uc','background-color','','');
	//SET RADIO TEXT COLOR	
		change_color('set-radio-text-color','data-checked-icon-color','color','','');
	//SET RADIO BG COLOR
	...
	jQuery(document).on('click','.select-search button',
			function()
				{
				jQuery('.select-search button').removeClass('active');
				jQuery(this).addClass('active');
				
				current_field.find('select').attr('data-select-search','no');
				
				if(jQuery(this).hasClass('select-search-yes'))
					current_field.find('select').attr('data-select-search','yes');
				}
			);
...
//PANEL SETTINGS
	...
	//SET PANEL ID
		jQuery(document).on('keyup','div.field-settings-column #set_panel_id',
			function()
				{
				var formated_value = format_illegal_chars(jQuery(this).val());
				jQuery(this).val(formated_value);
				current_field.find('div.panel').first().attr('data-panel-id',jQuery(this).val())
				}
			);
	...
	//SET PANEL HEADING TEXT
		jQuery(document).on('keyup','div.field-settings-column #set_panel_heading',
			function()
				{
				current_field.find('div.panel-heading').first().html(jQuery(this).val())
				}
			);
	//SET PANEL BIU
...
//SETUP VALIDATION SETTINGS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
	...
	//SET MAX WORDS
		jQuery(document).on('keyup','div.field-settings-column #set_max_word_val',
			function()
				{
				input_element.attr('data-max-words',jQuery(this).val())
				}
			);
	//SET MIN WORDS
		jQuery(document).on('keyup','div.field-settings-column #set_min_word_val',
			function()
				{
				input_element.attr('data-min-words',jQuery(this).val())
				}
			);		
	...
	//SET INPUT MASK
	 ...
	 jQuery(document).on('change','#text_transform',
			function()
				{
				
				input_element.attr('data-text-transform','');
				
				
				if(jQuery(this).val()=='none')
					input_element.removeAttr('data-text-transform');
				else
					input_element.attr('data-text-transform',jQuery(this).val());
