	...
	var set_options =''; 
		field_container.find(target).each(
			function(i)
				...
				var get_radio_style = '';
				if(jQuery(this).find('.input-label'))
					{
					if(jQuery(this).find('.input-label').attr('style'))
						get_label_style = jQuery(this).find('.input-label').attr('style');
					}
				
				if(jQuery(this).find('a:not(.checked)').attr('style'))
				 	get_radio_style = jQuery(this).find('a:not(.checked)').attr('style');
				...
				if(theme=='bootstrap' || theme=='m_design')
					...
					if(radio_check_type=='radio')
						...
						set_options += '<label class="radio-inline  '+ set_ui_theme + ' ' + set_layout +'" for="'+ format_illegal_chars(label) +'" ><span class="svg_ready"><input class="radio the_input_element" type="radio" name="'+ format_illegal_chars(group_name) +'" id="'+format_illegal_chars(label)+'" value="'+value+'"><span class="input-label radio-label" style="'+ get_label_style +'">'+label+'</span></span></label>';					
						}
					if(radio_check_type=='check')
						...
						set_options += '<label class="checkbox-inline '+ set_ui_theme + ' ' + set_layout +'" for="'+ format_illegal_chars(label) +'" ><span class="svg_ready"><input class="check the_input_element" type="checkbox" name="'+ format_illegal_chars(group_name) +'" id="'+format_illegal_chars(label)+'" value="'+value+'"><span class="input-label check-label" style="'+ get_label_style +'">'+label+'</span></span></label>';
						}
					}
...
/*///////////////READY FUNCTION///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
/*////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
jQuery('title').text('NEX-Forms');
jQuery(document).ready(
function()
	{
	
	jQuery('title').text('NEX-Forms - '+jQuery('input#form_name').val());
	jQuery(document).on('keyup', 'input#form_name', function(){
		jQuery('title').html('NEX-Forms-'+ jQuery(this).val());
	});	
	
	var get_total_steps = jQuery('.nex-forms-container .form_field.step').length;
					
		if(get_total_steps!=0)
			{
	...
	get_overall_form_settings(jQuery(jQuery('.form-canvas-area').attr('data-sec-pre-class')+' .inner-canvas-container')) 
	jQuery('.form_loader').hide();
	jQuery('.form_loader').remove();
	
	jQuery('.width_bar').addClass('animated');
	jQuery('.width_bar').addClass('fadeInDown');
	jQuery('.width_bar').show();
	
	//setTimeout(function(){	
	jQuery('.width_input').addClass('animated');
	jQuery('.width_input').addClass('fadeInDown');
	...
	jQuery('.outer-container').addClass('fadeInUp');
	jQuery('.outer-container').show();
	//},0);
	
	jQuery('.bc-outer-container').addClass('animated');
	jQuery('.bc-outer-container').addClass('fadeInUp');
	jQuery('.bc-outer-container').show();
	setTimeout(function(){	
		jQuery('.width_bar').removeClass('fadeInDown').removeClass('animated');
		jQuery('.outer-container').removeClass('fadeInUp').removeClass('animated');
		jQuery('.bc-outer-container').removeClass('fadeInUp').removeClass('animated');
		jQuery('.width_input').removeClass('fadeInDown').removeClass('animated');
	},2000);
	...
	jQuery(document).on('mouseenter','.dash-left-col', //
			function()
				{
				var left_col = jQuery(this);
				open_menu = setTimeout(function()
					{
					left_col.addClass('opened');
					jQuery('.dash-right-col').addClass('opened');
					},500);
				}
			);
		jQuery(document).on('mouseleave','.dash-left-col', //
			function()
				{
				clearTimeout(open_menu);
				jQuery(this).removeClass('opened');
				jQuery('.dash-right-col').removeClass('opened');
				}
			);
	...
	jQuery(document).on('click','.close-preview',
		function()
			{
			jQuery('.nex_forms_admin_page_wrapper').removeClass('full_preview');
			...
			jQuery('.canvas_view.workspace.normal').trigger('click');
			}
		);
	...
	jQuery(document).on('click','.prime-preview.preview',
		function()
			{
			jQuery('.prime-preview.preview .top-icon').attr('class','top-icon fa fa-spin fa-refresh');
			jQuery('.laptop').trigger('click');
			jQuery('.show_form_preview').hide();
			setTimeout(
						function()
							{
							jQuery('.preview-tools .btn.workspace.preview').trigger('click');
							}
							,10
						);
	...
	jQuery(document).on('click','.preview-tools .btn.workspace',
		function()
			...
			if(jQuery(this).hasClass('c-logic'))
				...
				jQuery('.nex_forms_admin_page_wrapper').addClass('full_preview');
				jQuery('.nex_forms_admin_page_wrapper').addClass('p-full');
	...
	jQuery(document).on('click', '.tabs_nf2 a', function(e){
		e.preventDefault();
		
		jQuery('.dash-right-col .nf-screen').addClass('hidden');
		
		jQuery('.dash-right-col #'+jQuery(this).attr('data-screen')).removeClass('hidden');
		
		jQuery('.tabs_nf2 li').removeClass('current');
		jQuery(this).parent().addClass('current');
	});
	
	
	jQuery(document).on('click', '.sub-screen', function(e){
		e.preventDefault();
		jQuery(this).closest('.form_attr_wrapper').find('.nf-sub-screen').addClass('hidden');
		jQuery(this).closest('.form_attr_wrapper').find('#'+jQuery(this).attr('data-screen')).removeClass('hidden').show();
	});
	
	
	jQuery(document).on('click', '.alt-tab a', function(){
		jQuery('.preview-tools .btn.workspace').removeClass('active');
		jQuery('.preview-tools .btn.workspace').removeClass('active');
			jQuery(this).addClass('active');	
			//jQuery('.conditional_logic_wrapper #close-settings').trigger('click');
		...
		if(jQuery(this).hasClass('success-msg'))
				jQuery('.preview-tools .success-msg').trigger('click');
	});
	
	
	jQuery(document).on('click', '.prime-menu .tabs_nf2 a', function(){
		jQuery('.prime-menu .tabs_nf2 .tab').removeClass('current');
		jQuery(this).parent().addClass('current');
	});
	
	jQuery(document).on('click', '.tabs_nf2 a', function(){
...
//INTEGRATION SETUP MENU CLICK	
	...
	jQuery(document).on('click', '.show_hubspot_setup', function(){
		jQuery('.tri-menu .show_hubspot_setup_menu_item a').trigger('click');
	});
...
function generate_shortcode(){
	...
	if(jQuery('.form_embed_shortcode_display').hasClass('no_code') && jQuery('.form_embed_types a.active').hasClass('popup'))
		{
		shortcode = '<div class="alert alert-danger"><span class="fas fa-lock"></span> PREMIUM ONLY FEATURE: An active premium license is required to enable Popup Forms.<br><a href="https://basixonline.net/nex-forms/pricing/" class="upgrade-link" target="_blank"> Upgrade to Premium <span class="fa-solid fa-angles-up"></span></a></div>';
		php_code = '<div class="alert alert-danger"><span class="fas fa-lock"></span> PREMIUM ONLY FEATURE: An active premium license is required to enable Popup Forms.<br> <a href="https://basixonline.net/nex-forms/pricing/" class="upgrade-link" target="_blank"> Upgrade to Premium <span class="fa-solid fa-angles-up"></span></a></div>';
		}
...
//OPTIONS SETUP MENU CLICK	
				...
				if(input.prop('type') === 'radio')
					...
					if(input.prop('checked'))
						...
						if(nexCheckable.closest('.the-radios').attr('data-checked-bg-color') && nexCheckable.closest('.the-radios').attr('data-checked-bg-color')!='')
							...
							nexCheckable.css('background','#2979ff');
	...
	jQuery(document).on('click','.resposive_tests i',
		function()
			...
			if(jQuery(this).hasClass('laptop'))
				...
				{
				jQuery('.nex_forms_admin_page_wrapper').removeClass('p-full');
			...
			if(jQuery('.nex_forms_admin_page_wrapper').hasClass('full_preview'))
				{
					jQuery('.show_form_preview').hide();
					jQuery('.show_form_preview').attr('src',jQuery('.admin_url').text() + '/admin.php?page=nex-forms-preview&form_Id=0');
					nf_save_nex_form('','preview', '');
	...
	jQuery(document).on('click','.canvas_background_tools .shade div',
		...
		jQuery(document).on('click','.tabs_nf2 .tab', function()
			{
		...
		jQuery(document).on('click',' a.email_setup, a.integration', function()
				{
				setup_tags();
				}
			);
		...
		jQuery(document).on('click','a.email_setup',
			function()
				{
				possible_email_fields();
	...
	jQuery(document).on('change', 'input[name="form_post_action"]', 
		function()
			...
			if(jQuery(this).val()=='ajax')
				...
				jQuery('.submit_to_form').addClass('hidden');
			...
			else if(jQuery(this).val()=='ftf')
				{
				jQuery('.submit_custom_options').addClass('hidden');
				jQuery('.submit_ajax_options').addClass('hidden');
				jQuery('.on_submit_redirect').addClass('hidden');
				jQuery('.submit_to_form').removeClass('hidden');
...
function nf_count_multi_steps(){
	...
	jQuery('.nex-forms-container .form_field.step').each(
		function(index, element)
			...
			set_stepping += '<li class="'+ is_current +'"><a data-title="'+ step_name +'"data-toggle="tooltip_bs" data-placement="bottom" data-show-step="'+ (index+1) +'">'+ (index+1) +'</a></li>';
...
function nf_apply_font(obj, selector){	
	  ...
	  if(jQuery('select[name="'+ selector +'"]').val()=='remove_fonts')
	  	{
			jQuery('select[name="'+ selector +'"]').attr('data-selected','')
			obj.css('font-family','');
			jQuery( '.nex-forms-container link' ).remove();
			return;
		}
	  ...
	  if ( 'undefined' !== font.name ) {
			if(!jQuery('link[id="'+ format_illegal_chars(font.name) +'"]').length>0)
				...
				&& format_illegal_chars(font.name)!='verdana'
					...
					{
					jQuery( '<link id="'+format_illegal_chars(font.name)+'" type="text/css" role="google-font-import" rel="stylesheet" href="https://fonts.googleapis.com/css?family='+ font.name +'">').prependTo( '.nex-forms-container' );
					}
				}
		}
...
function nf_save_nex_form(form_id,form_status, clicked_obj)
	...
	if(clicked_obj)
		...
		clicked_obj.html('<span class="fas fa-spin fa-refresh"></span>');
		}
...
let fields_data_array = [];

jQuery('div.admin_html #field_container .the_input_element, div.admin_html .set_math_result').each(function (index) {
    const $field = jQuery(this);
    const tagName = $field.get(0).tagName;
    
    var fieldName = $field.attr('name');
    var fieldId = $field.closest('.form_field').attr('id');
    var fieldType = $field.attr('type');
    var fieldValue = $field.attr('value');
    var fieldSettings = jQuery(this).closest('.form_field').attr('data-settings');
	
	if(fieldName!='math_result')
		{
		if (tagName === 'INPUT' || tagName === 'TEXTAREA' || tagName === 'SELECT') {
			...
			if (fieldType === 'radio' || fieldType === 'checkbox') {
				// Check if the radio button group already exists
				let existingRadioGroup = fields_data_array.find(item => item.field_name === fieldName);
			
				if (existingRadioGroup) {
					// Ensure field_value is an array
					if (!Array.isArray(existingRadioGroup.field_value)) {
						existingRadioGroup.field_value = [existingRadioGroup.field_value]; // Convert single value to array
					}
					// Add the new value if it isn't already in the array
					if (!existingRadioGroup.field_value.includes(fieldValue)) {
						existingRadioGroup.field_value.push(fieldValue);
					}
				} else {
					// Create a new entry for the radio button group
					fields_data_array.push({
						field_Id: fieldId, // Keep the first radio button's ID
						field_name: fieldName,
						field_type: fieldType,
						field_value: [fieldValue], // Initialize as an array
				...
				$field.find('option').each(function () {
					selectOptions.push(jQuery(this).val()); // Push each option value into the array
				});
				...
				fields_data_array.push({
					...
					field_name: fieldName,
					field_type: 'select',
					field_value: selectOptions // Array of dropdown option values
				});
			} 
			else if (tagName === 'TEXTAREA') {
				var set_field_type = 'textarea';
	
				if (fieldSettings && fieldSettings.includes('.s-sigs')) {
					set_field_type = 'signature';
				}
				
				fields_data_array.push({
					field_Id: fieldId,
					field_name: fieldName,
					field_type: set_field_type,
...
console.log(fields_data_array);
						...
						function(index)
							...
							if(jQuery(this).find('.input-group'))
								...
								if(jQuery('#'+ jQuery(this).find('select[name="cla_field"] option:selected').attr('data-field-id')).hasClass('step') && jQuery(this).find('select[name="the_action"]').val()=='show')
									{
									//jQuery('.nf_ms_breadcrumb li:nth-child(2)').addClass('hidden_by_logic');
									...
									var get_step_num = parseInt(clean_html.find('#'+ jQuery(this).find('select[name="cla_field"] option:selected').attr('data-field-id')).attr('data-step-num'));
									jQuery('.nf_ms_breadcrumb li:nth-child('+get_step_num+')').addClass('hidden_by_logic')
									//console.log(get_step_num);
									}
	...
	multistep_settings.push(
			...
			show_front_end: (jQuery('.nex-forms-container .form_field.step').length<1) ? 'no' : jQuery('.nf_ms_breadcrumb ul').attr('data-show-front-end'),
	...
	option_settings.push(
			...
			form_expire:jQuery('input[name="form_expire"]').val(),
	...
	pdf_settings.push(
			{
			pdf_orientation:jQuery('input[name="pdf_orientation"]:checked').val(),
			pdf_size:(jQuery('select[name="pdf_size"]').val()) ? jQuery('select[name="pdf_size"]').val() : 'A4',
			}
		);
	...
	if(clean_html.find('.autocomplete.special_fields').attr('class'))
	 ...
	 if(clean_html.find('.select.selection_fields').attr('class'))
		inc_pro_select = 'true';
		
	 if(clean_html.find('.multi-select.selection_fields').attr('class'))
		inc_pro_select = 'true';
	...
	clean_html.find('.math_fields').each(
		function()
			{
			if(jQuery(this).find('.the_input_element').attr('data-math-equation'))
				inc_math = 'true';
	...
	enqueue_array.push(
			...
			is_inc_pro_select:inc_pro_select,
			}
		);
	...
	var data =
		...
		form_url							: jQuery('#nex_form_url').val(),
		...
		hubspot_portal_id					: jQuery('input[name="hubspot_portal_id"]').val(),
		hubspot_form_id						: jQuery('input[name="hubspot_form_id"]').val(),
		is_hubspot							: jQuery('input[name="integrate_hubspot"]:checked').val(),
		field_details						: fields_data_array
		};
		if(clicked_obj)
			{
	...
	jQuery.post
		...
		ajaxurl, data, function(response)
			...
			if(form_status=='preview')
				{
				var preview_type = 'split';
				
				var preview_device = 'desktop';
				
				if(jQuery('.nex_forms_admin_page_wrapper').hasClass('full_preview'))
					preview_type = 'full';
					
				if(!jQuery('.nex_forms_admin_page_wrapper').hasClass('p-full'))
					preview_device = 'mobile';
				
				
				jQuery('.show_form_preview').fadeIn().attr('src',jQuery('.admin_url').text() + '/admin.php?page=nex-forms-preview&p-type='+ preview_type +'&p-device='+ preview_device +'&preview-form-id='+ jQuery('#form_update_id').text() +'&form_Id='+response);
				if(clicked_obj)
				...
				setTimeout(
						function()
							...
							jQuery('.prime-preview.preview .top-icon').attr('class','top-icon fas fa-eye');
							}
							,300
						);
					...
					if(is_template=='1')
						...
						if(jQuery('#form_update_id').text())
							...
							if(jQuery('#form_type').text()=='template')
								...
								setTimeout(function()
									{ 
									jQuery('.prime_save').html('<span class="top-icon fas fa-save"></span><span class="menu-text">SAVE</span>'); //jQuery('.prime_save').removeClass('flip_btn'); 
									},2000);
								}
							}
						else
