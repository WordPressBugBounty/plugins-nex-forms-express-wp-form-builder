...
/* INSERT */
		...
		public function insert_record(){
			...
			$fields 	= $wpdb->get_results('SHOW FIELDS FROM ' . $wpdb->prefix .sanitize_text_field($db_table));
			$field_array = array();
			...
			$insert = $wpdb->insert ( $wpdb->prefix . sanitize_text_field($db_table), $field_array );
...
/* UPDATE */
		...
		public function update_record(){
			...
			$update = $wpdb->update ( $wpdb->prefix . sanitize_text_field($db_table), $field_array, array(	'Id' => $edit_id) );
			echo sanitize_title($edit_id);
			wp_die();
		}
	...
	public function update_paypal(){
		...
		if(!$get_row)
			...
			$update = $wpdb->update ( $wpdb->prefix . sanitize_text_field($db_table), $field_array, array(	'nex_forms_Id' => sanitize_title($nex_forms_Id)) );
			
			
		echo sanitize_title($nex_forms_Id);
		die();
		}
	...
	public function build_paypal_products($form_Id){
		...
		$form = $wpdb->get_row('SELECT * FROM '. $wpdb->prefix .'wap_nex_forms WHERE Id = '.sanitize_title($form_Id));
...
/* DUPLICATE */
		public function duplicate_record(){
			...
			$get_fields 	= $wpdb->prepare("SHOW FIELDS FROM " . $wpdb->prefix .sanitize_text_field($db_table),'');
			...
			$insert = $wpdb->prepare ( $wpdb->insert ( $wpdb->prefix . sanitize_text_field($db_table), $field_array ),'');
			$wpdb->query($insert);
...
/* DELETE */
		public function delete_record(){
			...
			$delete = $wpdb->delete($wpdb->prefix. sanitize_text_field($db_table),array('Id'=>$record_id));	
			$delete_draft = $wpdb->delete($wpdb->prefix. sanitize_text_field($db_table),array('draft_Id'=>$record_id));	
			wp_die();
		}	
		public function delete_file(){
			...
			$record_id = sanitize_title($_POST['Id']);
			$get_file = $wpdb->prepare('SELECT location FROM ' .$wpdb->prefix. sanitize_text_field($db_table). ' WHERE Id = %d',$record_id);
		...
		public function NEXForms_get_data(){
				...
				echo sanitize_text_field($license_data['message']);
				die();
		}
/* ALTER TABLE */
...
/* PREVIEW FORM */
		public function preview_nex_form(){
			...
			if($form)
				...
				$update = $wpdb->update ( $wpdb->prefix . sanitize_text_field($db_table), $field_array, array(	'Id' => sanitize_title($edit_id)) );
				echo sanitize_title($edit_id);
			...
			foreach($forms as $form)
				...
				$output .= '<tr id="'.$form->Id.'" class="'.$is_template.'">';
					...
					$output .= '<td align="right">';
						$output .= '<a class="nf-button export_form" data-toggle="tooltip2" data-placement="left" title="" data-original-title="Export"  href="'.get_option('siteurl').'/wp-admin/admin.php?page=nex-forms-main&nex_forms_Id='.$form->Id.'&export_form=true"><span class="fa fa-cloud-download bs-tooltip"  data-toggle="tooltip2" data-placement="left" title="" data-original-title="Export"></span></a>';
					
						$output .= '<a class="duplicate_record nf-button" data-toggle="tooltip2" data-placement="top" title="Duplicate" id="'.$form->Id.'">&nbsp;<span class="fa fa-files-o"></span>&nbsp;</button>';
					
						$output .= '<a id="'.$form->Id.'" class="do_delete nf-button" data-toggle="tooltip2" data-placement="top" title="" data-original-title="Delete">&nbsp;<span class="fa fa-trash"></span>&nbsp;</button>';
					$output .= '</td>';	
				$output .= '</tr>';	
	...
	public function get_title($Id='',$table=''){
			...
			return sanitize_text_field($the_title);
	...
	public function get_title2($Id='',$table=''){
			...
			$get_the_title = $wpdb->prepare("SELECT title FROM " . $wpdb->prefix .$table." WHERE Id = %d",sanitize_title($Id));
			$the_title = $wpdb->get_var($get_the_title);
	...
	public function get_user_firstname($Id){
			global $wpdb;
			$get_username = $wpdb->prepare("SELECT meta_value FROM " . $wpdb->prefix . "usermeta WHERE user_id = %d AND meta_key='first_name'",sanitize_title($Id));
	...
	public function get_user_lastname($Id){
			global $wpdb;
			$get_username = $wpdb->prepare("SELECT meta_value FROM " . $wpdb->prefix . "usermeta WHERE user_id = %d AND meta_key='last_name'",sanitize_title($Id));
			$username = $wpdb->get_var($get_username);
			return $username;
		}	
	...
	public function get_username($Id){
			global $wpdb;
			$get_username = $wpdb->prepare("SELECT display_name FROM " . $wpdb->prefix . "users WHERE ID = %d",sanitize_title($Id));
			$username = $wpdb->get_var($get_username);
			return $username;
		}
	public function get_useremail($Id){
			global $wpdb;
			$get_useremail = $wpdb->prepare("SELECT user_email FROM " . $wpdb->prefix . "users WHERE ID = %d",sanitize_title($Id));
			$useremail = $wpdb->get_var($get_useremail);
			return $useremail;
		}
	public function get_userurl($Id){
			global $wpdb;
			$get_userurl = $wpdb->prepare("SELECT user_url FROM " . $wpdb->prefix . "users WHERE ID = %d",sanitize_title($Id));
	...
	public function load_nex_form(){
			...
			$get_form = $wpdb->get_row($wpdb->prepare('SELECT form_fields FROM '.$wpdb->prefix.'wap_nex_forms WHERE '.$get_id.'= %d',sanitize_title($form_Id)));
	...
	public function load_conditional_logic($form_Id=''){
			...
			$get_logic = $wpdb->prepare('SELECT conditional_logic FROM '.$wpdb->prefix.'wap_nex_forms WHERE Id=%d ',sanitize_title($form_Id));
...
';
		...
		public function load_conditional_logic_array($form_Id=''){
			...
			$get_logic = $wpdb->prepare('SELECT conditional_logic_array FROM '.$wpdb->prefix.'wap_nex_forms WHERE Id=%d ',sanitize_title($form_Id));
			$conditional_logic = $wpdb->get_var($get_logic);
	...
	//echo '</pre>';
		...
		public function get_email_setup(){
			...
			if($_POST['form_Id'])
				...
				$form = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms WHERE '.$get_id.'=%d',sanitize_title($form_Id)));
				}
	//ADMIN EMAIL SETUP
		...
		public function get_options_setup(){
			...
			if($_POST['form_Id'])
				...
				$form = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms WHERE '.$get_id.'=%d',$form_Id));
				}
			$preferences = get_option('nex-forms-preferences');	
	//FORM ATTR
	...
	/*** From Address ***/	
		...
		public function load_form_entries(){
			...
			$results 	= $wpdb->get_results($wpdb->prepare('SELECT * FROM '. $wpdb->prefix . 'wap_nex_forms_entries WHERE Id <> "" 
											...
											'.(($plugin_alias) ? ' AND (plugin="'.sanitize_text_field($plugin_alias).'" || plugin="shared")' : '').' 
											'.$where_str.'   
											ORDER BY 
											'.((isset($orderby) && !empty($orderby)) ? sanitize_text_field($orderby).' 
											'.sanitize_text_field($orderby) : 'Id DESC').' 
											LIMIT '.((isset($current_page)) ? $current_page*10 : '0'  ).',10 ',''));
		...
		public function populate_form_entry(){
			...
			$form_entry_Id = sanitize_title($_POST['form_entry_Id']);
			 if(isset($_POST['batch']) && $_POST['batch']=='false')
				 	{
					$form_entry = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_entries WHERE Id= %d',$form_entry_Id));
			...
			if($checkout)
				{					
				 ...
				 $update = $wpdb->update($wpdb->prefix.'wap_nex_forms_entries',array('viewed'=>'viewed'), array('Id'=>$form_entry_Id));
	...
	public function save_mc_key() {
		...
		update_option('nex_forms_mailchimp_api_key',sanitize_text_field($api_key));
	...
	public function save_gr_key() {
		...
		update_option('nex_forms_get_response_api_key',sanitize_text_field($api_key));
	...
	public function save_email_config() {
		...
		update_option('nex-forms-email-config',array
			(
			'email_method'			=> sanitize_text_field($email_method),
			'smtp_host' 			=> sanitize_text_field($smtp_host),
			'mail_port' 			=> sanitize_text_field($mail_port),
			'email_smtp_secure' 	=> sanitize_text_field($email_smtp_secure),
			'smtp_auth' 			=> $smtp_auth,
			'set_smtp_user' 		=> sanitize_text_field($set_smtp_user),
			'set_smtp_pass' 		=> sanitize_text_field($set_smtp_pass),
			'email_content' 		=> sanitize_text_field($email_content)
			)
	...
	public function save_script_config() {
		...
		update_option('nex-forms-script-config',array
			(
			'inc-jquery' 					=> sanitize_title($inc_jquery),
			'inc-jquery-ui-core' 			=> sanitize_title($inc_jquery_ui_core),
			'inc-jquery-ui-autocomplete' 	=> sanitize_title($inc_jquery_ui_autocomplete),
			'inc-jquery-ui-slider' 			=> sanitize_title($inc_jquery_ui_slider),
			'inc-jquery-form' 				=> sanitize_title($inc_jquery_form),
			'inc-bootstrap' 				=> sanitize_title($inc_bootstrap),
			'inc-onload' 					=> sanitize_title($inc_onload),
			'enable-print-scripts' 			=> sanitize_title($enable_print_scripts),
			'inc-moment' 					=> sanitize_title($inc_moment),
			'inc-locals' 					=> sanitize_title($inc_locals),
			'inc-datetime' 					=> sanitize_title($inc_datetime),
			'inc-math' 						=> sanitize_title($inc_math),
			'inc-colorpick' 				=> sanitize_title($inc_colorpick),
			'inc-wow' 						=> sanitize_title($inc_wow),
			'inc-raty' 						=> sanitize_title($inc_raty),
			'inc-sig' 						=> sanitize_title($inc_sig)
			)
		);
		die();
	}
	...
	public function save_style_config() {
		...
		update_option('nex-forms-style-config',array
			(
			'incstyle-jquery' 					=> sanitize_title($incstyle_jquery),
			'incstyle-font-awesome' 			=> sanitize_title($incstyle_font_awesome),
			'incstyle-font-awesome-v4-shims' 	=> sanitize_title($incstyle_font_awesome_v4),
			'incstyle-bootstrap' 				=> sanitize_title($incstyle_bootstrap),
			'incstyle-custom' 					=> sanitize_title($incstyle_custom),
			'incstyle-animations' 				=> sanitize_title($incstyle_animations),
			'enable-print-styles' 				=> sanitize_title($enable_print_styles)
			)
		);
		die();
	}
	...
	public function do_form_import() {
		...
		if($_POST['import_type']=='manual')
			...
			$pattern2 = '/url\((.*?)\)/'; // Match url(...) and capture the content inside the parentheses
			$outputString2 = preg_replace_callback($pattern2, function($matches) {
				return str_replace('"', '', $matches[0]); // Remove double quotes from the matched content
			}, $outputString);
...
/**************************************************/
		...
		mail($email_address,$subject,$set_body,$headers);
		}
...
/**************************************************/	
		...
		if($email_config['email_content']=='html')	
			...
			$set_body = $plain_body;
		$email_address = sanitize_email($_POST['email_address']);
		wp_mail($email_address,$subject,$set_body,$headers);				
		}
